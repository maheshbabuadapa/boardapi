...
ENVIRONMENTS = ['dev', 'sit', 'uat', 'preprod', 'prod', 'de']
...
def display_logs(selected_rows, rows, env):
    ...
    logs_dict = response.json().get('logs', {})
    if isinstance(logs_dict, dict):
        log_text = "\n\n".join([f"=== {pod} ===\n{log}" for pod, log in logs_dict.items()])
        return log_text or 'No logs available.'
    else:
        return logs_dict
